...
int duration = 10; // Simulation time 운영시간
...
int served_customers; // Number of customers served serve한 고객
...
int remove_customer()
{
	...
	printf("_Customer %d starts service in %d minutes. Wait time was %d minutes.\n", customer.id, clock, clock - customer.arrival_time);
	return service_time;
}
	...
	while (clock < duration) {
		...
		for(i=0;i<num_of_staff;i++){
			if (service_time_mat[i] > 0)	// the customer is receiving service
				...
				service_time_mat[i] = remove_customer();
		}
